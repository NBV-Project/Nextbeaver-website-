...
const useAnimationLoop = (
  ...
  }, [trackRef, targetVelocity, seqWidth, seqHeight, isHovered, hoverSpeed, isVertical, isActive]);
};
  ...
  ({
    ...
    const cssVariables = useMemo(
      () =>
        ({
          '--logoloop-gap': `${gap}px`,
          '--logoloop-logoHeight': `${logoHeight}px`, // Use prop value here
    ...
    const rootClasses = useMemo(
      () =>
        cx(
          ...
          '[--logoloop-gap:32px]',
          // Removed hardcoded [--logoloop-logoHeight:28px] here
      ...
      (item: LogoItem, key: React.Key) => {
        if (renderItem) {
          return (
            <li
              className={cx(
                'flex-none h-[var(--logoloop-logoHeight)] text-[length:var(--logoloop-logoHeight)] leading-[1]',
        ...
        const content = isNodeItem ? (
          ...
          <Image
            ...
            alt={item.alt ?? ''}
            title={item.title}
            width={item.width ?? Math.max(1, Math.round(logoHeight * 3))}
            height={item.height ?? Math.max(1, Math.round(logoHeight))}
              sizes={item.sizes ?? "160px"}
              loading="lazy"
              decoding="async"
              draggable={false}
              unoptimized={isSvgSource(item.src)}
              style={{ width: "auto" }}
            />
        );
