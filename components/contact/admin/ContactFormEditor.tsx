...
import type { ContactFormContent, ContactStyles } from "@/lib/supabase/contact";
...
type Props = {
  ...
  lang: "en" | "th";
  mode: "light" | "dark";
  onChange: (content: ContactFormContent, styles: ContactStyles) => void;
};
...
export default function ContactFormEditor({ content, styles, lang, mode, onChange }: Props) {
  const [activeIndex, setActiveIndex] = useState<number | null>(null);
  const swipeStartX = useRef<number | null>(null);
  const swipeDeltaX = useRef(0);

  const updateContent = (field: keyof ContactFormContent, value: string) => {
    onChange({ ...content, [field]: value }, styles);
  };

  const updateContentText = (base: string, value: string) => {
    const field = (lang === "th" ? `${base}_th` : base) as keyof ContactFormContent;
    ...
    () => [
      {
        ...
        label: "ป้ายกำกับชื่อ",
        base: "nameLabel",
        placeholderBase: "namePlaceholder",
        type: "text",
        fontBase: "formLabelFontFamily",
        sizeKey: "formLabelFontSize" as keyof ContactStyles,
        colorBase: "formLabelColor",
        extra: [
          { label: "สีพื้นหลังฟอร์ม", base: "formBg" },
          { label: "สีขอบฟอร์ม", base: "formBorder" },
        ]
      },
      {
        id: "companyLabel",
        label: "ป้ายกำกับบริษัท",
        base: "companyLabel",
        placeholderBase: "companyPlaceholder",
        type: "text",
        fontBase: "formLabelFontFamily",
        sizeKey: "formLabelFontSize" as keyof ContactStyles,
        colorBase: "formLabelColor",
      ...
      {
        id: "emailLabel",
        label: "ป้ายกำกับอีเมล",
        base: "emailLabel",
        placeholderBase: "emailPlaceholder",
        type: "text",
        fontBase: "formLabelFontFamily",
        sizeKey: "formLabelFontSize" as keyof ContactStyles,
        colorBase: "formLabelColor",
      ...
      {
        id: "detailsLabel",
        label: "ป้ายกำกับรายละเอียด",
        base: "detailsLabel",
        placeholderBase: "detailsPlaceholder",
        type: "text",
        fontBase: "formLabelFontFamily",
        sizeKey: "formLabelFontSize" as keyof ContactStyles,
        colorBase: "formLabelColor",
      ...
      {
        id: "inputStyles", // Dummy field for input styles
        label: "ช่องกรอกข้อมูล",
        base: "namePlaceholder", // Just to bind to something
        isShared: true, // Mock shared to avoid localized logic on dummy
        type: "hidden", // We will render special UI
        fontBase: "inputFontFamily",
        sizeKey: "inputFontSize" as keyof ContactStyles,
        colorBase: "inputText",
        extra: [
          { label: "สีพื้นหลังช่อง", base: "inputBg" },
          { label: "สีขอบช่อง", base: "inputBorder" },
          { label: "สีข้อความตัวอย่าง", base: "inputPlaceholder" },
        ],
      },
      {
        id: "submitLabel",
        ...
        base: "submitLabel",
        type: "text",
        fontBase: "buttonFontFamily",
        sizeKey: "buttonFontSize" as keyof ContactStyles,
        colorBase: "buttonText",
        extra: [
          { label: "สีพื้นหลังปุ่ม", base: "buttonBg" },
        ],
      },
    ],
    []
            ...
            return (
              ...
              >
                <div className="space-y-1">
                  ...
                  <div className="text-sm text-white/80 line-clamp-1">{field.type === "hidden" ? "ปรับแต่งดีไซน์" : value || "—"}</div>
                </div>
        ...
        >
          <div className="w-full max-w-none sm:max-w-md max-h-[84vh] sm:max-h-[58vh] overflow-y-auto rounded-none sm:rounded-2xl border border-[#3a2f1d] bg-[#1a1612] shadow-[0_30px_80px_rgba(0,0,0,0.55)]">
            ...
            >
              ...
              >
                {fields.map((field) => {
                  const value = !field.isShared ? getContentText(field.base) : "";
                  const placeholderValue = field.placeholderBase ? getContentText(field.placeholderBase) : "";
                  ...
                  return (
                    <div key={field.id} className="w-full px-6 py-5 space-y-4">
                      {field.type !== "hidden" && (
                        <>
                          <div className="space-y-1">
                            <label className="text-[10px] uppercase tracking-wider font-semibold text-[#cbb790]/60">ข้อความป้ายกำกับ</label>
                            <input
                          ...
                          {field.placeholderBase && (
                            <div className="space-y-1">
                              <label className="text-[10px] uppercase tracking-wider font-semibold text-[#cbb790]/60">ข้อความตัวอย่าง (Placeholder)</label>
                              <input
                                className="w-full bg-[#120f0b] border border-white/10 rounded-lg px-4 py-3 text-sm text-white"
                                value={placeholderValue}
                                onChange={(e) => updateContentText(field.placeholderBase!, e.target.value)}
                              />
                            </div>
                          )}
                        </>
